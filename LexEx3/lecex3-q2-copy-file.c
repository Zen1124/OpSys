...
void * copy_file( void * arg )
{
  ...
  int * bytes_copied = calloc( 1, sizeof( int ) );

  int infd, outfd;
  int bytes_written = 0;

  printf( "CHILD THREAD: Copying \"%s\" to backup\n", (char *)arg );

  infd = open( arg, O_RDONLY );
  if ( infd == -1 )
  {
    perror( "CHILD THREAD: open() failed to read existing file" );
    pthread_exit( bytes_copied );
  }
  ...
  if ( outfd == -1 )
  {
    perror( "CHILD THREAD: open() failed to create backup file" );
    pthread_exit( bytes_copied );
  }
